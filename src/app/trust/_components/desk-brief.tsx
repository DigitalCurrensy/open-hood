...
export interface DeskBriefCopy {
  eyebrow: string;
  dummy: { job: string; for: string; click: string; say: string };
  genius: { job: string; for: string; click: string; say: string };
}

export function DeskBrief({ brief, beginnerHint, expertHint }: { brief: DeskBriefCopy; beginnerHint: string; expertHint: string }) {
  return (
    <section aria-label="Page brief" className="rounded-sm border border-white/10 bg-bay-2/80">
      <div className="flex flex-wrap items-end justify-between gap-2 border-b border-white/10 px-4 py-2.5">
        ...
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-aluminum">
          <span data-label-dummy>{beginnerHint}</span>
          <span data-label-shop>{expertHint}</span>
        </p>
      </div>
