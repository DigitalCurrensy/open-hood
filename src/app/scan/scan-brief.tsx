import { SCAN_BRIEF } from "@/config/nav/scan";
  ...
  return (
    <section aria-label="Page brief" className="rounded-sm border border-white/10 bg-bay-2/80">
      <div className="flex flex-wrap items-end justify-between gap-2 border-b border-white/10 px-4 py-2.5">
        <p className="font-mono text-[11px] uppercase tracking-[0.32em] text-cone">{SCAN_BRIEF.eyebrow}</p>
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-aluminum">
          <span data-label-dummy>Beginner · park · type</span>
          <span data-label-shop>Expert · ATZ · PID · Mode 03</span>
        </p>
      </div>
        ...
        {ANSWERS.map((answer) => (
          <div key={answer.key} className="min-w-0">
            ...
            <dd className="page-brief-answer mt-1 text-sm leading-6 text-aluminum">
              <span data-altitude="dummy">{SCAN_BRIEF.dummy[answer.key]}</span>
              <span data-altitude="genius">{SCAN_BRIEF.genius[answer.key]}</span>
            </dd>
          </div>
        ))}
      </dl>
