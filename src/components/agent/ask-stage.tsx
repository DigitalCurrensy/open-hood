...
export function AskStage() {
  ...
  return (
    <section id="ask" aria-label="Ask the advocate" className="overflow-hidden rounded-sm border-2 border-ticket bg-bay-2">
      <header className="flex flex-col gap-3 border-b border-white/10 px-4 py-4 sm:flex-row sm:items-end sm:justify-between sm:px-6">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.32em] text-cone">The box you came for</p>
          <h2 className="mt-1 font-display text-4xl uppercase leading-none text-fluorescent sm:text-5xl">Ask</h2>
          <p className="mt-2 max-w-xl text-sm leading-6 text-aluminum">
            Type the quote line, the noise, or the scanner code. You get sentences for the counter — not a shop booking.
          </p>
      ...
      <div ref={scroller} className="min-h-[10rem] max-h-[22rem] space-y-3 overflow-y-auto px-4 py-4 sm:px-6" aria-live="polite">
        {messages.length === 0 ? (
          <p className="text-sm leading-6 text-aluminum">
            Example: “They quoted $89 for a cabin filter.” or “What oil does this take?”
          </p>
        ) : null}
        {messages.map((row) => (
      ...
      <div className="flex flex-wrap gap-1.5 border-t border-white/10 px-4 py-3 sm:px-6">
        {QUICK_PROMPTS.map((prompt) => (
          <button
            ...
            className="rounded-sm border border-white/15 px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-[0.12em] text-aluminum hover:border-ticket/50 hover:text-fluorescent disabled:opacity-50"
          >
            {prompt.label}
          </button>
        ))}
      ...
      >
        <label className="block">
          ...
          <textarea
            ...
            rows={3}
            autoFocus
            placeholder="They quoted a cabin filter… it squeals when I brake… scanner says P0420…"
        ...
        {error ? <p className="mt-2 text-sm text-cone">{error}</p> : null}
        <button
          type="submit"
          disabled={busy}
          className="mt-3 inline-flex min-h-12 w-full items-center justify-center rounded-sm bg-ticket px-4 py-3 font-mono text-sm font-semibold uppercase tracking-[0.18em] text-ticket-ink disabled:opacity-50 sm:w-auto"
        >
          {busy ? "Writing the brief…" : "Ask"}
        </button>
      </form>
    </section>
  );
